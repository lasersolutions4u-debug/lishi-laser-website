...
BODY_TRANSLATIONS = {
    'zh': {
        ...
        'label_global': '全球覆盖',
        'h2_global': '设备部署在哪里',
        'global_subtitle': '当前及近期部署。以下数字反映的是实际安装或正在进行的现场测试 — 不是预期或预测。',
        'stat_power': '功率范围',
        'stat_langs': '支持语言',
        'stat_power_use': '24小时功耗',
        'badge_china': '中国（工厂现场）',
        'badge_overseas': '海外客户',
        'global_note': '目前大部分部署在中国客户的工厂。我们处于服务海外客户的早期阶段 — 如果您正在评估该设备是否适合您的设施，请联系我们进行坦诚的讨论。',
        'label_why': '为什么选择我们',
    ...
    'es': {
        ...
        'label_global': 'ALCANCE GLOBAL',
        'h2_global': 'Dónde Están Desplegados los Dispositivos',
        'global_subtitle': 'Despliegues actuales y recientes. Los números a continuación reflejan lo que está realmente instalado o en pruebas de campo activas — no proyecciones.',
        'stat_power': 'Rango de Potencia',
        'stat_langs': 'Idiomas Soportados',
        'stat_power_use': 'Consumo por 24h',
        'badge_china': 'CHINA (SITIOS DE FÁBRICA)',
        'badge_overseas': 'CLIENTES EN EL EXTRANJERO',
        'global_note': 'La mayoría de los despliegues actuales están en fábricas de clientes en China. Estamos en una etapa inicial de servicio a clientes en el extranjero — si está evaluando el dispositivo para su instalación, contáctenos para una discusión honesta.',
    ...
    'de': {
        ...
        'label_global': 'GLOBALE REICHWEITE',
        'h2_global': 'Wo die Geräte eingesetzt sind',
        'global_subtitle': 'Aktuelle und kürzliche Einsätze. Die folgenden Zahlen spiegeln wider, was tatsächlich installiert ist oder in aktiver Feldtestung steht — keine Pipeline oder Prognosen.',
        'stat_power': 'Leistungsbereich',
        'stat_langs': 'Unterstützte Sprachen',
        'stat_power_use': 'Verbrauch pro 24h',
        'badge_china': 'CHINA (FABRIKSTANDORTE)',
        'badge_overseas': 'AUSLÄNDISCHE KUNDEN',
        'global_note': 'Die meisten aktuellen Einsätze befinden sich in Kundenfabriken in China. Wir sind in einer frühen Phase der Betreuung ausländischer Kunden — wenn Sie das Gerät für Ihre Anlage evaluieren, kontaktieren Sie uns für ein ehrliches Gespräch.',
        'label_why': 'Warum wir',
    ...
    'fr': {
        ...
        'label_global': 'PORTÉE MONDIALE',
        'h2_global': 'Où les Dispositifs Sont Déployés',
        'global_subtitle': 'Déploiements actuels et récents. Les chiffres ci-dessous reflètent ce qui est réellement installé ou en test terrain actif — pas des projections.',
        'stat_power': 'Gamme de Puissance',
        'stat_langs': 'Langues Prises en Charge',
        'stat_power_use': 'Consommation par 24h',
        'badge_china': 'CHINE (SITES DE FABRIQUE)',
        'badge_overseas': 'CLIENTS ÉTRANGERS',
        'global_note': 'La plupart des déploiements actuels sont dans des usines clientes en Chine. Nous en sommes aux débuts de notre service aux clients étrangers — si vous évaluez le dispositif pour votre installation, contactez-nous pour une discussion honnête.',
    ...
    'ja': {
        ...
        'label_global': 'グローバル展開',
        'h2_global': '装置の導入場所',
        'global_subtitle': '現在および最近の導入。以下の数値は、実際に設置されているか、アクティブなフィールドテスト中のものを反映しています — パイプラインや予測ではありません。',
        'stat_power': '出力範囲',
        'stat_langs': '対応言語',
        'stat_power_use': '24時間消費電力',
        'badge_china': '中国（工場サイト）',
        'badge_overseas': '海外顧客',
        'global_note': '現在の導入の大部分は中国の顧客工場にあります。海外顧客へのサービスは初期段階です — お客様の施設向けに装置を評価中の場合は、お気軽にご相談ください。',
        'label_why': '選ばれる理由',
    ...
    'ru': {
        ...
        'label_global': 'ГЛОБАЛЬНЫЙ ОХВАТ',
        'h2_global': 'Где развёрнуты устройства',
        'global_subtitle': 'Текущие и недавние внедрения. Приведённые цифры отражают фактически установленное или находящееся в активных полевых испытаниях оборудование — не планы или прогнозы.',
        'stat_power': 'Диапазон мощности',
        'stat_langs': 'Поддерживаемые языки',
        'stat_power_use': 'Потребление за 24ч',
        'badge_china': 'КИТАЙ (ЗАВОДСКИЕ ПЛОЩАДКИ)',
        'badge_overseas': 'ЗАРУБЕЖНЫЕ КЛИЕНТЫ',
        'global_note': 'Большинство текущих внедрений находятся на фабриках клиентов в Китае. Мы на раннем этапе обслуживания зарубежных клиентов — если вы оцениваете устройство для своего предприятия, свяжитесь с нами для честного обсуждения.',
        'label_why': 'Почему мы',
...
def apply_translations(content, lang):
    ...
    replacements = [
        ...
        # Global Reach
        ('>GLOBAL REACH<', f'>{bt.get("label_global", "GLOBAL REACH")}<'),
        ('>Where the Devices Are Deployed<', f'>{bt.get("h2_global", "Where the Devices Are Deployed")}<'),
        ('Current and recent deployments. The numbers below reflect what is actually installed or in active field testing — not pipeline or projections.', bt.get('global_subtitle', '')),
        ('>Power Range Covered<', f'>{bt.get("stat_power", "Power Range Covered")}<'),
        ('>Languages Supported<', f'>{bt.get("stat_langs", "Languages Supported")}<'),
        ('>Power Use per 24h<', f'>{bt.get("stat_power_use", "Power Use per 24h")}<'),
        ('CHINA (FACTORY SITES)', bt.get('badge_china', 'CHINA (FACTORY SITES)')),
        ('OVERSEAS CUSTOMERS', bt.get('badge_overseas', 'OVERSEAS CUSTOMERS')),
        ('Most current deployments are at customer factories in China. We are at the early stage of serving overseas customers — if you are evaluating the device for your facility, contact us for an honest discussion of whether it fits your setup.', bt.get('global_note', '')),
